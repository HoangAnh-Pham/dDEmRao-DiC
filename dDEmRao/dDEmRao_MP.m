...
% ------------------------------------------------------------------------%
function [xopt,fopt,exitflag,output,X,Fitness,history,FE,DI,S] = dDEmRao_MP(fname,fcons,nvars,LB,UB,DX,option,type,dir,DiC)
...
%            iter,num_FE,fbest,fmean)
       ...
       XB = X(1:NP1,:); FB = Fitness(1:NP1); CB = Const(1:NP1);
       XW = X(NP1+1:NP,:); FW = Fitness(NP1+1:NP); CW = Const(NP1+1:NP);
              
       % Loop over all solutions
       for k=1:NP
           % Take random vectors from group-1 solutions
           ID1 = 1:NP1;
           JK = randperm(round(NP1)); JK(ID1(JK)==k)=[]; 
           r1b = ID1(JK(1)); r2b = ID1(JK(2)); r3b = ID1(JK(3)); 

           % Take random vectors from group-2 solutions
           ID2 = NP1+1:NP;
           JK = randperm(round(NP2)); JK(ID2(JK)==k)=[]; 
           r1w = ID2(JK(1)); r2w = ID2(JK(2)); r3w = ID2(JK(3)); 

           if k<=NP1,
               r1 = r1b; r2 = r2b; r3 = r3b;
           else
               r1 = r1w; r2 = r2w; r3 = r3w;
           end
